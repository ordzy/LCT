...
#### macOS/Linux
> If you are using Linux, be sure to have the `Lunar Client-X.AppImage` renamed to `lunarclient` in `/usr/bin/`. Alternatively, run LCT with `Lunar Client Tweaks ~/path/to/lunar/appimage`.
...
3. Run the `Lunar Client Tweaks` executable
